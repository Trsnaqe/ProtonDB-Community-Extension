function shouldSkipAddingLine(item) {
    ...
    const specificClassesToCheck = [...ClassNamesToCheck, "animated_featured_capsule_Title_3vZJE", "title"];
    const specificClassesToCheckInParent = [...ClassNamesToCheckInParent];

    if (
        item.matches(".gutter_item") ||
        item.querySelector(".vertical-line") ||
        specificClassesToCheck.some((className) => hasAnyClass(item, [className])) ||
        specificClassesToCheckInParent.some((className) => hasAnyParentClass(item, [className]))
    ) {
        return true;
    }
...
function shouldSkipAddingIcon(item) {
    ...
    const specificClassesToCheck = [...ClassNamesToCheck, "animated_featured_capsule_Artwork_3UsQc", "capsule"];
    const specificClassesToCheckInParent = [...ClassNamesToCheckInParent];
...
function addLineToItem(item) {
    ...
    chrome.runtime.sendMessage({ contentScriptQuery: "queryGame", appId }, (data) => {
...
function addIconToItem(item, isItemInSearchBox) {
    ...
    chrome.runtime.sendMessage({ contentScriptQuery: "queryDeckStats", appId }, (data) => {
        if (chrome.runtime.lastError) {
            console.log("Error happened:");
            console.log(`Error: ${chrome.runtime.lastError.message}`);
            return;
...
function isItemInTabList(item) {
    return item.classList.contains("tab_item");
}
