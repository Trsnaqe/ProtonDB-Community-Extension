...
function handleStorageMutation(changes, namespace) {
    ...
    if (changedTiers.length > 0) {
        const selector = changedTiers.map((tier) => `.vertical-line[data-tier="${tier}"]`).join(",");
        ...
        lines.forEach((line) => {
            ...
            const rankValueSpans = document.querySelectorAll(`.protondb_rank_value[data-tier="${lowercaseTier}"]`);
